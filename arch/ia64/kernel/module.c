...
{
	return addr - (uint64_t) mod->mem[MOD_INIT_TEXT].base < mod->mem[MOD_INIT_TEXT].size;
}

static inline int
in_core (const struct module *mod, uint64_t addr)
{
	return addr - (uint64_t) mod->mem[MOD_TEXT].base < mod->mem[MOD_TEXT].size;
}
	  ...
	  Elf64_Shdr *sec, void *location)
{
	...
	switch (formula) {
		...
		break;
	      ...
	      case RV_BDREL:
		val -= (uint64_t) (in_init(mod, val) ? mod->mem[MOD_INIT_TEXT].base : mod->mem[MOD_TEXT].base);
		break;
		    ...
		    unsigned int relsec, struct module *mod)
{
	...
	if (!mod->arch.gp) {
		 ...
		 */
		uint64_t gp;
		if (mod->mem[MOD_TEXT].size > MAX_LTOFF)
			/*
			 * This takes advantage of fact that SHF_ARCH_SMALL gets allocated
			 * at the end of the module.
			 */
			gp = mod->mem[MOD_TEXT].size - MAX_LTOFF / 2;
		else
			gp = mod->mem[MOD_TEXT].size / 2;
		gp = (uint64_t) mod->mem[MOD_TEXT].base + ((gp + 7) & -8);
		mod->arch.gp = gp;
