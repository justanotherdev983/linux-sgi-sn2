...
static const struct proc_ops salinfo_event_fops = {
...
retry:
	...
	if (!data->saved_num)
		work_on_cpu(cpu, salinfo_log_read_cpu, data);
	if (!data->log_size) {
...
salinfo_log_clear(struct salinfo_data *data, int cpu)
{
	...
	if (rh->severity != sal_log_severity_corrected)
		work_on_cpu(cpu, salinfo_log_clear_cpu, data);
...
salinfo_log_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	...
	if (sscanf(cmd, "read %d", &cpu) == 1) {
		...
		if (salinfo_platform_oemdata) {
			...
			count = work_on_cpu(cpu, salinfo_platform_oemdata_cpu,
						 &parms);
		} else
			data->oemdata_size = 0;
	} else
...
static const struct proc_ops salinfo_data_fops = {
