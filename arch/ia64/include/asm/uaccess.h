...
#define __get_user_size(val, addr, n, err)                                           \
	do {                                                                         \
		...
		asm("\n[1:]\tld" #n                                                  \
		    " %0=%2%P2\t// %0 and %1 get overwritten by exception handler\n" \
		    "\t.xdata4 \"__ex_table\", 1b-., 1f-.+4\n"                       \
		    "[1:]"                                                           \
		    : "=r"(__gu_r9), "=r"(__gu_r8)                                   \
		    : "m"(__m(addr)), "1"(__gu_r8));                                 \
		(err) = __gu_r8;                                                     \
		(val) = __gu_r9;                                                     \
	} while (0)
...
#define __put_user_size(val, addr, n, err)                                          \
	do {                                                                        \
		...
		asm volatile(                                                       \
			"\n[1:]\tst" #n                                             \
			" %1=%r2%P1\t// %0 gets overwritten by exception handler\n" \
			"\t.xdata4 \"__ex_table\", 1b-., 1f-.\n"                    \
			"[1:]"                                                      \
			: "=r"(__pu_r8)                                             \
			: "m"(__m(addr)), "rO"(val), "0"(__pu_r8));                 \
		(err) = __pu_r8;                                                    \
	} while (0)
...
#define __do_get_user(check, x, ptr, size)                             \
	({                                                             \
		...
		if (!check || __access_ok(__gu_ptr, size))             \
			switch (__gu_size) {                           \
			case 1:                                        \
				__get_user_size(__gu_val, __gu_ptr, 1, \
						__gu_err);             \
				break;                                 \
			case 2:                                        \
				__get_user_size(__gu_val, __gu_ptr, 2, \
						__gu_err);             \
				break;                                 \
			case 4:                                        \
				__get_user_size(__gu_val, __gu_ptr, 4, \
						__gu_err);             \
				break;                                 \
			case 8:                                        \
				__get_user_size(__gu_val, __gu_ptr, 8, \
						__gu_err);             \
				break;                                 \
			default:                                       \
				__get_user_unknown();                  \
...
#define __do_put_user(check, x, ptr, size)                           \
	({                                                           \
		...
		if (!check || __access_ok(__pu_ptr, __pu_size))      \
			switch (__pu_size) {                         \
			case 1:                                      \
				__put_user_size(__pu_x, __pu_ptr, 1, \
						__pu_err);           \
				break;                               \
			case 2:                                      \
				__put_user_size(__pu_x, __pu_ptr, 2, \
						__pu_err);           \
				break;                               \
			case 4:                                      \
				__put_user_size(__pu_x, __pu_ptr, 4, \
						__pu_err);           \
				break;                               \
			case 8:                                      \
				__put_user_size(__pu_x, __pu_ptr, 8, \
						__pu_err);           \
				break;                               \
			default:                                     \
				__put_user_unknown();                \
 ...
 * strlen.
 */
extern long __must_check __strncpy_from_user(char *to, const char __user *from,
					     long to_len);
...
#define strncpy_from_user(to, from, n)                                      \
	({                                                                  \
		...
		if (__access_ok(__sfu_from, 0))                             \
			__sfu_ret =                                         \
				__strncpy_from_user((to), __sfu_from, (n)); \
		__sfu_ret;                                                  \
	})
...
static __inline__ void *xlate_dev_mem_ptr(phys_addr_t p)
{
	struct page *page;
	void *ptr;
 ...
 */
static __inline__ void *xlate_dev_kmem_ptr(void *p)
{
	struct page *page;
	void *ptr;
